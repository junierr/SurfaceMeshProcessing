...
double MeshTools::AverageC(const Mesh & mesh)
{
	if (mesh.faces_empty()) return 0.0;
	double len = 0.0;
	int cnt = 0;
	for (const auto & f : mesh.faces()) {
		cnt++;
		auto he = mesh.halfedge_handle(f);
		len += mesh.calc_edge_length(he);
...
double MeshTools::AverageAera(const Mesh & mesh)
{
	if (mesh.faces_empty()) return 0.0;
	double s = 0.0;
	int cnt = 0;
...
double MeshTools::getcircle(const Mesh &mesh)
{
	if (mesh.faces_empty()) return 0.0;
	double x = 0.0, y = 0.0, z = 0.0;
	//double maxx = 0.0, maxy = 0.0, maxz = 0.0;
	double nowx = 0.0, nowy = 0.0, nowz = 0.0;
	double lenth = 0.0, nowlenth = 0.0;
	...
	for (const auto& vh : mesh.vertices())
	{
		auto point = mesh.point(vh);
			...
			nowy = point.data()[1];
			nowz = point.data()[2];
			nowlenth = sqrt((nowx - x)*(nowx - x) + (nowy - y)*(nowy - y) + (nowz - z)*(nowz - z));
			if (nowlenth> lenth)
			{
				//maxx = nowx;
				//maxy = nowy;
...
/*void MeshTools::drawnormal(const Mesh &mesh)
{
	if (mesh.points_empty()) return;
