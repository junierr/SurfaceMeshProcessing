...
auto MeshViewerWidget::getPoint(double u, double v,double lenth)
{
	double x = sin(PI*v)*cos(PI2*u);
	double y = sin(PI*v)*sin(PI2*u);
	double z = cos(PI*v);
	return Mesh::Point(x*lenth, y*lenth, z*lenth);
}


void  MeshViewerWidget::drawWire(double lenth)
{
		...
		{
			auto a = MeshViewerWidget::getPoint(0, 0, lenth);
			glVertex3dv(a.data());
			auto b = MeshViewerWidget::getPoint(u, vstep, lenth);
			glVertex3dv(b.data());
			auto c = MeshViewerWidget::getPoint(u + ustep, vstep, lenth);
			glVertex3dv(c.data());
			u += ustep;
		}
		//绘制中间四边形组
		...
		for (int i = 1; i < vStepNum - 1; i++)
		{
			...
			{
				auto a = getPoint(u, v, lenth);
				auto b = getPoint(u + ustep, v, lenth);
				auto c = getPoint(u + ustep, v + vstep, lenth);
				auto d = getPoint(u, v + vstep, lenth);
				glVertex3dv(a.data());
				glVertex3dv(b.data());
				glVertex3dv(c.data());
				glVertex3dv(d.data());
		...
		for (int i = 0; i < uStepsNum; i++)
		{
			auto a = getPoint(0, 1, lenth);
			auto b = getPoint(u, 1 - vstep, lenth);
			auto c = getPoint(u + ustep, 1 - vstep, lenth);
			glVertex3dv(a.data());
			glVertex3dv(b.data());
			glVertex3dv(c.data());
		}
...
void MeshViewerWidget::UpdateMesh(void)
{
	...
	std::cout << "AverageArea=:" << MeshTools::AverageAera(mesh) << std::endl;
	std::cout<<"AverageC=:"<< MeshTools::AverageC(mesh)<< std::endl;
...
void MeshViewerWidget::SetDrawCircle(bool b)
{
	isDrawCircle = b;
	update();
}
void MeshViewerWidget::SetDrawNormal(bool b)
{
	isDrawNormal = b;
	update();
}
...
void MeshViewerWidget::DrawCircle(void) 
{
	double lenth = MeshTools::getcircle(mesh);
	//std::cout << lenth << std::endl;
	MeshViewerWidget::drawWire(lenth);
}

void MeshViewerWidget::DrawNormal(void)
{
	...
	for (const auto&vh : mesh.vertices())
	{
		auto point1 = mesh.point(vh);
		Mesh::Normal Normals=mesh.normal(vh);
		auto point2 = Mesh::Point(point1.data()[0] + Normals.data()[0] * 0.02, point1.data()[1] + Normals.data()[1] * 0.02, point1.data()[2] + Normals.data()[2] * 0.02);
